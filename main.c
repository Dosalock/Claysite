...
Clay_RenderCommandArray
	UpdateDrawFrame ( float width,
					  ...
					  bool arrow_key_left_pressed_this_frame,
					  bool arrow_key_right_pressed_this_frame,
					  bool shift_key_pressed_this_frame,
					  ...
					  float delta_time )
{
	...
	if ( is_mouse_down
		 && Clay_PointerOver( Clay_GetElementIdWithIndex(
			 CLAY_STRING( "set_information_button" ),
			 1 ) ) )
	{
	...
	if ( pressed_last_frame == false )
	{
		// TODO FIX NEGATIVE BOUNDS
		if ( shift_key_pressed_this_frame )
		{
			if ( arrow_key_left_pressed_this_frame )
			{
				input_index_substring_length  = input_index_substring_length - 1;
				pressed_last_frame            = true;
			}
			if ( arrow_key_right_pressed_this_frame )
			{
				input_index_substring_length  = input_index_substring_length + 1;
				pressed_last_frame            = true;
			}
	...
	else if ( arrow_key_right_pressed_this_frame == false
			  && arrow_key_left_pressed_this_frame == false )
	{
		pressed_last_frame = false;
	}
	RenderHighlight( textbox_id, input_index, input_index_substring_length );
